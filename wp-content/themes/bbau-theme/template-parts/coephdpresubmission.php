<?php
/**
 * Template Name: PhD Pre-Submission Seminars Page
 */
defined('ABSPATH') || exit;

$api_base   = getenv('DJANGO_API_URL');
$media_base = getenv('DJANGO_MEDIA_URL');
$BBAU_PHD_PRESUB_API = $media_base . '/api/v1/coe/phd-pre-submission-seminars/';

$notices       = array();
$notices_error = '';

$cache_key = 'bbau_phd_presub_seminars_all';
...
if (!empty($notices)) {
    foreach ($notices as $n) {
        if (!empty($n['date'])) {
            $ts = strtotime($n['date']);
            if ($ts) {
                $bbau_presub_years[(int) date('Y', $ts)] = true;
            }
        }
    }
    krsort($bbau_presub_years);
    $bbau_presub_years = array_keys($bbau_presub_years);
}
...
<section class="container-fluid page-bg py-5">
    ...
    <div class="container">
        ...
        <?php elseif (empty($notices)): ?>
            ...
            <div class="alert alert-warning">No pre-submission seminars available.</div>
        ...
        <?php else: ?>
            ...
            <div class="pb-filter-bar">
                ...
                <select id="pbYearFilter" class="pb-filter-select">
                    ...
                    <?php foreach ($bbau_presub_years as $y): ?>
